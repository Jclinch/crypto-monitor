...
export default function CalculatorPage() {
  ...
  if (!geckoCoins || !cmcCoins || !ccPrices || !binancePrices) {
    return <p className="p-6">Loading live prices...</p>;
  }
  ...
  return (
    <main className="p-6 bg-gray-50 text-gray-950 min-h-screen">
      <h1 className="text-3xl font-bold mb-6">Crypto Calculator</h1>
      ...
      <div className="flex flex-wrap gap-4 mb-6">
        ...
        <select
          ...
          onChange={(e) => setCurrency(e.target.value as "usd" | "ngn")}
          className="border p-2 rounded-md"
          disabled={mode === "crypto"} // currency only matters in fiat mode
        >
          <option value="usd">USD</option>
          <option value="ngn">NGN</option>
        </select>
        ...
        <select
          value={mode}
          onChange={(e) => setMode(e.target.value as "fiat" | "crypto")}
          className="border p-2 rounded-md"
        >
          <option value="fiat">Fiat → Crypto</option>
          <option value="crypto">Crypto → Fiat</option>
        </select>
      </div>
        ...
        <thead className="bg-gray-100">
          <tr>
            ...
            <th className="p-2 text-left">
              {mode === "fiat" ? `You Get (${selectedCoin})` : `You Pay (${currency.toUpperCase()})`}
            </th>
          </tr>
        </thead>
        <tbody>
          {results.map((r) => (
            <tr key={r.platform} className="border-t">
              ...
              <td className="p-2">
                {r.price
                  ? mode === "fiat"
                    ? (amount / (typeof r.price === "number" ? r.price : 1)).toFixed(6) // Fiat → Crypto
                    : (amount * (typeof r.price === "number" ? r.price : 1)).toLocaleString() // Crypto → Fiat
                  : "N/A"}
              </td>
            </tr>
          ))}
